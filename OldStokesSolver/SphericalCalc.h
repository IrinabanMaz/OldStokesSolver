...
*/
inline double dot(const RectCoord& a, const RectCoord& b)
{
...
*/
inline RectCoord cross(const RectCoord& a, const RectCoord& b)
{
    RectCoord temp;
...
*/
inline std::ostream& operator <<(std::ostream& o, const RectCoord& x)
{
    o << "[" << x.x << ", " << x.y << ", " << x.z << "]";
    return o;
}
inline bool operator ==(const RectCoord& x, const RectCoord& y)
{
...
{
    return v * std::cos(angle) + cross(axis, v) * std::sin(angle) + axis * dot(axis, v) * (1.0 - std::cos(angle));
}
...
inline SphereCoord RectToSphere(const RectCoord& x, const RectCoord& c = RectCoord(), const SurfaceCoord& axis = SurfaceCoord(), const double& radius = 1.0)
{
    ...
    RectCoord ephi(-std::sin(axis.phi), std::cos(axis.phi), 0);

    RectCoord xr = rotate(x - c, ephi, -axis.theta);

    temp.rho = std::sqrt(dot(xr, xr)) / radius;

    temp.s.phi = std::atan2(xr.y, xr.x);

    double planenorm = std::sqrt((xr.x) * (xr.x) + (xr.y) * (xr.y));

    temp.s.theta = std::atan2(planenorm, xr.z);
...
{
    RectCoord temp;
    temp.x = r.rho * std::sin(r.s.theta) * std::cos(r.s.phi);
    temp.y = r.rho * std::sin(r.s.theta) * std::sin(r.s.phi);
    temp.z = r.rho * std::cos(r.s.theta);

    RectCoord ephi(-std::sin(r.northPole.phi), std::cos(r.northPole.phi), 0);
...
double LInfNorm(SphereData& data, int& maxglindex, int& maxtrapindex, const MathConstants& consts = defaultconsts)
{
